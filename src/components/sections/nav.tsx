...
const Nav = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      const sections = ["hero", "features", "testimonial", "carousel", "contact"];
  ...
  return (
    <nav className="fixed top-6 left-6 right-6 w-auto z-[9999] flex items-center justify-center">
      <div className="flex items-center justify-center h-[3rem] bg-green-600/20 px-1 py-2 rounded-full border border-green-700">
        <ul className="flex gap-0.5 mx-auto">
          {[
            ...
            { id: "features", icon: "arrow", label: "Features" },
            { id: "testimonial", icon: "feather", label: "Testimonials" },
            { id: "carousel", icon: "currency", label: "Gallery" },
            { id: "contact", icon: "contact", label: "Contact" },
          ].map((item) => (
            <li key={item.id}>
              <button
                onClick={() => handleClick(item.id)}
                className={`relative flex items-center justify-center px-4 py-2 text-lg cursor-pointer font-medium outline-none transition-all duration-300 ${
              ...
              >
                <span
                  className={`absolute inset-0 bg-green-700 mix-blend-difference rounded-full transition-all duration-300 ${
